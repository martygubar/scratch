...
select systimestamp as Begin from dual;
-- Recreate custsales
...
alter table custsales add (
  actual_price binary_double as (list_price - (list_price * discount_percent))
);  
...
/*            case  
                when (paid_m1 + paid_m2 + paid_m3 + paid_m4) / nullif((views_m1 + views_m2 + views_m3),0) - (paid_total) / nullif((views_total),0) > 0 then 'up'
                when (paid_m1 + paid_m2 + paid_m3 + paid_m4) / nullif((views_m1 + views_m2 + views_m3),0) - (paid_total) / nullif((views_total),0) < 0 then 'down'
                else 'flat'
            end paid_ratio_trend,       
            case  
                when (discount_m1 + discount_m2 + discount_m3 + discount_m4) / nullif((views_m1 + views_m2 + views_m3 + views_m4),0) - (discount_total) / nullif((views_total),0) > 0 then 'up'
                when (discount_m1 + discount_m2 + discount_m3 + discount_m4) / nullif((views_m1 + views_m2 + views_m3 + views_m4),0) - (discount_total) / nullif((views_total),0) < 0 then 'down'
                else 'flat'
            end discount_ratio_trend
*/         round((paid_m1 + paid_m2 + paid_m3 + paid_m4) / nullif((views_m1 + views_m2 + views_m3 + views_m4),0) - (paid_total) / nullif((views_total),0), 2) as paid_ratio_trend,
           round((discount_m1 + discount_m2 + discount_m3 + discount_m4) / nullif((views_m1 + views_m2 + views_m3 + views_m4),0) - (discount_total) / nullif((views_total),0),2) as discount_ratio_trend
        from agg_base
    )
select 
    cust_id,
...
--    age,
    ...
    customer_segment,
    views_total,
    views_m3 as views_oct,
    views_m4 as views_sep,
    views_m5 as views_aug,
    views_m6 as views_jul,
    views_monthly_avg,
    views_m3_pr_month_ratio as views_oct_pr_month_ratio,
    paid_total,
    paid_ratio_trend,
    paid_m3 as paid_oct,
    paid_m4 as paid_sep,
    ...
    discount_m3 as discount_oct,
    discount_m4 as discount_sep,
    discount_m5 as discount_aug,
    discount_m6 as discount_jul,
    action_adventure,
    family_friendly,
    docu_crime,
    drama_romance,
...
select cust_id, month, is_profile
from profile sample(70);  -- original 70, 65
...
-- Shift churners purchases back several months
update custsales
set day_id = add_months(day_id, -1 * trunc(dbms_random.value(3,12)))
where cust_id in (select cust_id from all_churners)
and to_char(day_id, 'YYYY-MM') in ('2020-12','2020-11');

commit;

-- Make action/adventure a more dominant set of categories for churners
-- ISSUE: shifting the movies made all the non-action aventure movies causal factors 
-- TODO: Instead of shifting, increase the # of action-adventure movies

-- create a table with action_adventure/movie combinations. This will be 
-- used to reallocate movies to these genres for churners
/* drop table genre_movie;

create table genre_movie as
select distinct genre_id, movie_id, list_price, discount_percent
from custsales
where genre_id in (1,2,6,20,22);

drop table custsales_shift_movies;

create table custsales_shift_movies as 
    select cust_id, genre_id, movie_id, day_id, discount_percent 
    from custsales sample(15)
    where cust_id in (select cust_id from all_churners)
      and genre_id not in (1,2,6,20,22);
    
update custsales
set (genre_id, movie_id, list_price) = 
    (select genre_id, movie_id, list_price
     from genre_movie
     order by dbms_random.value
     fetch first 1 rows only)
where (cust_id, genre_id, movie_id, day_id) in 
        (select cust_id, genre_id, movie_id, day_id
         from custsales_shift_movies);

commit;
*/
/*
update custsales
set actual_price = list_price - (list_price * discount_percent);
where (cust_id, day_id) in 
        (select cust_id, day_id
         from custsales_shift_movies);

commit;
*/

-- Reduce the discount churners who meet the profile were receiving starting in Sept
drop table cust_remove_discount;

create table cust_remove_discount as 
select cust_id, day_id
from custsales
where (
        cust_id in (select cust_id from all_churners sample(70)) 
        or
        cust_id in (select cust_id from cust_scratch sample(10))   
      )
...
select systimestamp as Ending from dual;
